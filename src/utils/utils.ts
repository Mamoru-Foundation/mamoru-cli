...
import { LaunchOptions } from '../commands/agents/launch'
import { input } from '@inquirer/prompts'
import { PublishOptions } from '../commands/agents/publish'
...
export async function queryDaemonParameters(
    ...
    const chainType = chainName
    const parameters = metadata.parameters.filter((el) => {
        if (el.hiddenFor.map((el) => el.name).includes(chainType)) return false
        return true
    })
